...
class DefaultDrmSession<T extends ExoMediaCrypto> implements DrmSession<T> {
        ...
        if (i == 0 || i == 1) {
            if (this.offlineLicenseKeySetId == null) {
                ...
                if (this.mode == 0 && licenseDurationRemainingSec <= 60) {
                    Log.m167d(TAG, "Offline license has expired or will expire soon. Remaining seconds: " + licenseDurationRemainingSec);
                    postKeyRequest(2, z);
    ...
    private boolean restoreKeys() {
        try {
            ...
            Log.m170e(TAG, "Error trying to restore Widevine keys.", e);
            onError(e);
            return false;
        }
    }
    ...
    private long getLicenseDurationRemainingSec() {
        if (!C1379C.WIDEVINE_UUID.equals(this.uuid)) {
    ...
    private void postKeyRequest(int i, boolean z) {
        try {
            ...
            this.postRequestHandler.post(1, keyRequest, z);
        } catch (Exception e) {
            onKeysError(e);
        }
    }
