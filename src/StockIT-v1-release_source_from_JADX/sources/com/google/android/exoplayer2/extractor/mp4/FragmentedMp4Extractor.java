...
public final class FragmentedMp4Extractor implements Extractor {
    ...
    public FragmentedMp4Extractor(int i, TimestampAdjuster timestampAdjuster2, Track track, DrmInitData drmInitData, List<Format> list, TrackOutput trackOutput) {
        ...
        this.pendingSeekTimeUs = C1379C.TIME_UNSET;
        this.segmentIndexEarliestPresentationTimeUs = C1379C.TIME_UNSET;
    ...
    private void onMoovContainerAtomRead(Atom.ContainerAtom containerAtom) throws ParserException {
        ...
        while (i5 < size2) {
            ...
            if (containerAtom3.type == Atom.TYPE_trak) {
                ...
                if (parseTrak != null) {
                    sparseArray2.put(parseTrak.f125id, parseTrak);
                }
            } else {
                i = i5;
                i2 = size2;
        ...
        if (this.trackBundles.size() == 0) {
            while (i3 < size3) {
                ...
                trackBundle.init(track, getDefaultSampleValues(sparseArray, track.f125id));
                this.trackBundles.put(track.f125id, trackBundle);
        ...
        while (i3 < size3) {
            ...
            this.trackBundles.get(track2.f125id).init(track2, getDefaultSampleValues(sparseArray, track2.f125id));
            i3++;
        }
    }
    ...
    private void onMoofContainerAtomRead(Atom.ContainerAtom containerAtom) throws ParserException {
        ...
        if (this.pendingSeekTimeUs != C1379C.TIME_UNSET) {
    ...
    private boolean readSample(ExtractorInput extractorInput) throws IOException, InterruptedException {
        ...
        if (this.parserState == 3) {
            if (this.currentTrackBundle == null) {
                ...
                if (position2 < 0) {
                    Log.m173w(TAG, "Ignoring negative offset to sample data.");
                    position2 = 0;
                }
    ...
    private static DrmInitData getDrmInitDataFromAtoms(List<Atom.LeafAtom> list) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (leafAtom.type == Atom.TYPE_pssh) {
                ...
                if (parseUuid == null) {
                    Log.m173w(TAG, "Skipped pssh atom (failed to extract uuid)");
                } else {
