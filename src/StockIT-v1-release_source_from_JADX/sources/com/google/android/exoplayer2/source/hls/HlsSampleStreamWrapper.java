...
final class HlsSampleStreamWrapper implements Loader.Callback<Chunk>, Loader.ReleaseCallback, SequenceableLoader, ExtractorOutput, SampleQueue.UpstreamFormatChangedListener {
    ...
    public Loader.LoadErrorAction onLoadError(Chunk chunk, long j, long j2, IOException iOException, int i) {
        ...
        boolean z = false;
        boolean maybeBlacklistTrack = blacklistDurationMsFor != C1379C.TIME_UNSET ? this.chunkSource.maybeBlacklistTrack(chunk2, blacklistDurationMsFor) : false;
        if (maybeBlacklistTrack) {
            ...
            createRetryAction = retryDelayMsFor != C1379C.TIME_UNSET ? Loader.createRetryAction(false, retryDelayMsFor) : Loader.DONT_RETRY_FATAL;
        }
    ...
    private static Format deriveFormat(Format format, Format format2, boolean z) {
        ...
        return format2.copyWithContainerInfo(format.f120id, format.label, mediaMimeType, codecsOfType, i, format.width, format.height, format.selectionFlags, format.language);
    }
    ...
    private static DummyTrackOutput createDummyTrackOutput(int i, int i2) {
        Log.m173w(TAG, "Unmapped track with id " + i + " of type " + i2);
