...
public class PermissionsActivity extends Activity {
    ...
    private void requestPermission() {
        if (Build.VERSION.SDK_INT < 23) {
            finish();
            overridePendingTransition(C2410R.anim.onesignal_fade_in, C2410R.anim.onesignal_fade_out);
        } else if (!waiting) {
            waiting = true;
    ...
    private void showLocationPermissionSettings() {
        new AlertDialog.Builder(ActivityLifecycleHandler.curActivity).setTitle(C2410R.string.location_not_available_title).setMessage(C2410R.string.location_not_available_open_settings_message).setPositiveButton(C2410R.string.location_not_available_open_settings_option, new DialogInterface.OnClickListener() {
    ...
    static void startPrompt(boolean z) {
        if (!waiting && !answered) {
            ...
            C24064 r1 = new ActivityLifecycleHandler.ActivityAvailableListener() {
                public void available(Activity activity) {
                    if (!activity.getClass().equals(PermissionsActivity.class)) {
                        Intent intent = new Intent(activity, PermissionsActivity.class);
                        intent.setFlags(131072);
                        activity.startActivity(intent);
                        activity.overridePendingTransition(C2410R.anim.onesignal_fade_in, C2410R.anim.onesignal_fade_out);
                    }
                }
            };
