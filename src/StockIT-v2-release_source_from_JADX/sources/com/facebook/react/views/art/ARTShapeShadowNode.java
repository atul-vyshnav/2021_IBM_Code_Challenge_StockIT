...
public class ARTShapeShadowNode extends ARTVirtualNode {
    ...
    public boolean setupFillPaint(Paint paint, float f) {
        ...
        if (i2 != 0) {
            if (i2 != 1) {
                FLog.m104w(ReactConstants.TAG, "ART: Color type " + i2 + " not supported!");
            } else {
                int i3 = 5;
                if (fArr3.length < 5) {
                    FLog.m104w(ReactConstants.TAG, "[ARTShapeShadowNode setupFillPaint] expects 5 elements, received " + this.mBrushData.length);
                    return false;
                }
