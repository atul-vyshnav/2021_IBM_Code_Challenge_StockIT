...
public final class SsMediaSource extends BaseMediaSource implements Loader.Callback<ParsingLoadable<SsManifest>> {
    ...
    private SsMediaSource(SsManifest ssManifest, Uri uri, DataSource.Factory factory, ParsingLoadable.Parser<? extends SsManifest> parser, SsChunkSource.Factory factory2, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory2, LoadErrorHandlingPolicy loadErrorHandlingPolicy2, long j, Object obj) {
        Uri uri2;
        boolean z = false;
    ...
    private void processManifest() {
        ...
        if (j2 == Long.MAX_VALUE) {
            ...
            if (this.manifest.dvrWindowLengthUs != C1379C.TIME_UNSET && this.manifest.dvrWindowLengthUs > 0) {
                j2 = Math.max(j2, j - this.manifest.dvrWindowLengthUs);
            }
            long j3 = j2;
            long j4 = j - j3;
            long msToUs = j4 - C1379C.msToUs(this.livePresentationDelayMs);
            if (msToUs < MIN_LIVE_DEFAULT_START_POSITION_US) {
                msToUs = Math.min(MIN_LIVE_DEFAULT_START_POSITION_US, j4 / 2);
            }
            singlePeriodTimeline = new SinglePeriodTimeline(C1379C.TIME_UNSET, j4, j3, msToUs, true, true, this.tag);
        } else {
            long j5 = this.manifest.durationUs != C1379C.TIME_UNSET ? this.manifest.durationUs : j - j2;
