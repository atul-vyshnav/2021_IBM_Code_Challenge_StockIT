...
public abstract class MediaCodecRenderer extends BaseRenderer {
    ...
    public MediaCodecRenderer(int i, MediaCodecSelector mediaCodecSelector2, DrmSessionManager<FrameworkMediaCrypto> drmSessionManager2, boolean z, float f) {
        super(i);
        ...
        if (this.codec == null && (format2 = this.format) != null) {
            ...
            try {
                if (initCodecWithFallback(mediaCrypto, z)) {
                    ...
                    this.codecHotswapDeadlineMs = getState() == 2 ? SystemClock.elapsedRealtime() + 1000 : C1379C.TIME_UNSET;
                    resetInputBuffer();
                    resetOutputBuffer();
    ...
    public void releaseCodec() {
        this.codecHotswapDeadlineMs = C1379C.TIME_UNSET;
        resetInputBuffer();
        resetOutputBuffer();
        ...
        if (!this.availableCodecInfos.isEmpty()) {
            do {
                ...
                try {
                    ...
                    Log.m174w(TAG, "Failed to initialize decoder: " + peekFirst, e2);
    ...
    private List<MediaCodecInfo> getAvailableCodecInfos(boolean z) throws MediaCodecUtil.DecoderQueryException {
        ...
        if (decoderInfos.isEmpty() && z) {
            ...
            if (!decoderInfos.isEmpty()) {
                Log.m173w(TAG, "Drm session requires secure decoder for " + this.format.sampleMimeType + ", but no secure decoder available. Trying to proceed with " + decoderInfos + ".");
            }
        }
        return decoderInfos;
    }
    ...
    public boolean isReady() {
        return this.format != null && !this.waitingForKeys && (isSourceReady() || hasOutputBuffer() || (this.codecHotswapDeadlineMs != C1379C.TIME_UNSET && SystemClock.elapsedRealtime() < this.codecHotswapDeadlineMs));
    }
        ...
        if (this.format != null && Util.SDK_INT >= 23) {
            ...
            if (this.codecOperatingRate != codecOperatingRate2) {
                ...
                if (this.codec != null && this.codecReinitializationState == 0) {
                    int i = (codecOperatingRate2 > -1.0f ? 1 : (codecOperatingRate2 == -1.0f ? 0 : -1));
                    if (i == 0 && this.codecConfiguredWithOperatingRate) {
                        reinitializeCodec();
                    } else if (i == 0) {
                    } else {
