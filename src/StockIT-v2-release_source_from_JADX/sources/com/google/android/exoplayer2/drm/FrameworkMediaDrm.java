...
public final class FrameworkMediaDrm implements ExoMediaDrm<FrameworkMediaCrypto> {
    ...
    private FrameworkMediaDrm(UUID uuid2) throws UnsupportedSchemeException {
        ...
        Assertions.checkArgument(!C1379C.COMMON_PSSH_UUID.equals(uuid2), "Use C.CLEARKEY_UUID instead");
        this.uuid = uuid2;
        this.mediaDrm = new MediaDrm(adjustUuid(uuid2));
        if (C1379C.WIDEVINE_UUID.equals(uuid2) && needsForceWidevineL3Workaround()) {
    ...
    public byte[] provideKeyResponse(byte[] bArr, byte[] bArr2) throws NotProvisionedException, DeniedByServerException {
        if (C1379C.CLEARKEY_UUID.equals(this.uuid)) {
    ...
    public FrameworkMediaCrypto createMediaCrypto(byte[] bArr) throws MediaCryptoException {
        return new FrameworkMediaCrypto(new MediaCrypto(adjustUuid(this.uuid), bArr), Util.SDK_INT < 21 && C1379C.WIDEVINE_UUID.equals(this.uuid) && "L3".equals(getPropertyString("securityLevel")));
    }

    private static DrmInitData.SchemeData getSchemeData(UUID uuid2, List<DrmInitData.SchemeData> list) {
        boolean z;
        if (!C1379C.WIDEVINE_UUID.equals(uuid2)) {
            return list.get(0);
        }
    ...
    private static UUID adjustUuid(UUID uuid2) {
        return (Util.SDK_INT >= 27 || !C1379C.CLEARKEY_UUID.equals(uuid2)) ? uuid2 : C1379C.COMMON_PSSH_UUID;
    }
    ...
    private static byte[] adjustRequestInitData(java.util.UUID r2, byte[] r3) {
        /*
            ...
            java.util.UUID r0 = com.google.android.exoplayer2.C1379C.WIDEVINE_UUID
            boolean r0 = r0.equals(r2)
            if (r0 != 0) goto L_0x003e
        L_0x000e:
            java.util.UUID r0 = com.google.android.exoplayer2.C1379C.PLAYREADY_UUID
            boolean r0 = r0.equals(r2)
    ...
    private static String adjustRequestMimeType(UUID uuid2, String str) {
        return (Util.SDK_INT >= 26 || !C1379C.CLEARKEY_UUID.equals(uuid2) || (!MimeTypes.VIDEO_MP4.equals(str) && !MimeTypes.AUDIO_MP4.equals(str))) ? str : "cenc";
    }

    private static byte[] adjustRequestData(UUID uuid2, byte[] bArr) {
        return C1379C.CLEARKEY_UUID.equals(uuid2) ? ClearKeyUtil.adjustRequestData(bArr) : bArr;
    }
