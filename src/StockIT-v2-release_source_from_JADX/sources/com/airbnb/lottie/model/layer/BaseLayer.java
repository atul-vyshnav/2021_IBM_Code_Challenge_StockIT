...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    static BaseLayer forModel(Layer layer, LottieDrawable lottieDrawable2, LottieComposition lottieComposition) {
        switch (C07402.$SwitchMap$com$airbnb$lottie$model$layer$Layer$LayerType[layer.getLayerType().ordinal()]) {
            case 1:
    ...
    public void draw(Canvas canvas, Matrix matrix2, int i) {
        C0692L.beginSection(this.drawTraceName);
        if (!this.visible || this.layerModel.isHidden()) {
            C0692L.endSection(this.drawTraceName);
            return;
        }
        buildParentLayerListIfNeeded();
        C0692L.beginSection("Layer#parentMatrix");
        this.matrix.reset();
        this.matrix.set(matrix2);
        for (int size = this.parentLayers.size() - 1; size >= 0; size--) {
            this.matrix.preConcat(this.parentLayers.get(size).transform.getMatrix());
        }
        C0692L.endSection("Layer#parentMatrix");
        ...
        if (hasMatteOnThisLayer() || hasMasksOnThisLayer()) {
            C0692L.beginSection("Layer#computeBounds");
            ...
            C0692L.endSection("Layer#computeBounds");
            if (!this.rect.isEmpty()) {
                C0692L.beginSection("Layer#saveLayer");
                this.contentPaint.setAlpha(255);
                C0749Utils.saveLayerCompat(canvas, this.rect, this.contentPaint);
                C0692L.endSection("Layer#saveLayer");
                clearCanvas(canvas);
                C0692L.beginSection("Layer#drawLayer");
                drawLayer(canvas, this.matrix, intValue);
                C0692L.endSection("Layer#drawLayer");
                if (hasMasksOnThisLayer()) {
                    applyMasks(canvas, this.matrix);
                }
                if (hasMatteOnThisLayer()) {
                    C0692L.beginSection("Layer#drawMatte");
                    C0692L.beginSection("Layer#saveLayer");
                    C0749Utils.saveLayerCompat(canvas, this.rect, this.mattePaint, 19);
                    C0692L.endSection("Layer#saveLayer");
                    clearCanvas(canvas);
                    this.matteLayer.draw(canvas, matrix2, intValue);
                    C0692L.beginSection("Layer#restoreLayer");
                    canvas.restore();
                    C0692L.endSection("Layer#restoreLayer");
                    C0692L.endSection("Layer#drawMatte");
                }
                C0692L.beginSection("Layer#restoreLayer");
                canvas.restore();
                C0692L.endSection("Layer#restoreLayer");
            }
            recordRenderTime(C0692L.endSection(this.drawTraceName));
            return;
        }
        this.matrix.preConcat(this.transform.getMatrix());
        C0692L.beginSection("Layer#drawLayer");
    ...
    private void intersectBoundsWithMask(RectF rectF, Matrix matrix2) {
        ...
        if (hasMasksOnThisLayer()) {
            ...
            while (i < size) {
                ...
                int i2 = C07402.$SwitchMap$com$airbnb$lottie$model$content$Mask$MaskMode[mask2.getMaskMode().ordinal()];
                if (i2 != 1 && i2 != 2) {
    ...
    static /* synthetic */ class C07402 {
        ...
        static {
            ...
            L_0x0083:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.airbnb.lottie.model.layer.BaseLayer.C07402.<clinit>():void");
        }
    }
    ...
    private void applyMasks(Canvas canvas, Matrix matrix2) {
        C0692L.beginSection("Layer#saveLayer");
        C0749Utils.saveLayerCompat(canvas, this.rect, this.dstInPaint, 19);
    ...
    private void applyInvertedSubtractMask(Canvas canvas, Matrix matrix2, Mask mask2, BaseKeyframeAnimation<ShapeData, Path> baseKeyframeAnimation, BaseKeyframeAnimation<Integer, Integer> baseKeyframeAnimation2) {
        C0749Utils.saveLayerCompat(canvas, this.rect, this.dstOutPaint);
        canvas.drawRect(this.rect, this.contentPaint);
    ...
    private void applyIntersectMask(Canvas canvas, Matrix matrix2, Mask mask2, BaseKeyframeAnimation<ShapeData, Path> baseKeyframeAnimation, BaseKeyframeAnimation<Integer, Integer> baseKeyframeAnimation2) {
        C0749Utils.saveLayerCompat(canvas, this.rect, this.dstInPaint);
        this.path.set(baseKeyframeAnimation.getValue());
        this.path.transform(matrix2);
        this.contentPaint.setAlpha((int) (((float) baseKeyframeAnimation2.getValue().intValue()) * 2.55f));
        canvas.drawPath(this.path, this.contentPaint);
