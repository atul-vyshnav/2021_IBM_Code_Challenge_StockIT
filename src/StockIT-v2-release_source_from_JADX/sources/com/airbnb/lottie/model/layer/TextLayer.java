...
public class TextLayer extends BaseLayer {
    ...
    public void drawLayer(Canvas canvas, Matrix matrix2, int i) {
        ...
        if (baseKeyframeAnimation5 != null) {
            ...
            if (baseKeyframeAnimation6 != null) {
                ...
                this.strokePaint.setStrokeWidth(documentData.strokeWidth * C0749Utils.dpScale() * C0749Utils.getScale(matrix2));
            }
        }
        if (this.lottieDrawable.useTextGlyphs()) {
    ...
    private void drawTextGlyphs(DocumentData documentData, Matrix matrix2, Font font, Canvas canvas) {
        ...
        float f2 = f / 100.0f;
        float scale = C0749Utils.getScale(matrix2);
        String str = documentData2.text;
        float dpScale = documentData2.lineHeight * C0749Utils.dpScale();
    ...
    private void drawGlyphTextLine(String str, DocumentData documentData, Matrix matrix2, Font font, Canvas canvas, float f, float f2) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (fontCharacter != null) {
                ...
                float width = ((float) fontCharacter.getWidth()) * f2 * C0749Utils.dpScale() * f;
    ...
    private void drawTextWithFont(DocumentData documentData, Font font, Matrix matrix2, Canvas canvas) {
        ...
        if (typeface != null) {
            ...
            this.fillPaint.setTextSize(f * C0749Utils.dpScale());
            this.strokePaint.setTypeface(this.fillPaint.getTypeface());
            this.strokePaint.setTextSize(this.fillPaint.getTextSize());
            float dpScale = documentData.lineHeight * C0749Utils.dpScale();
            List<String> textLines = getTextLines(str);
            int size = textLines.size();
            for (int i = 0; i < size; i++) {
                String str2 = textLines.get(i);
    ...
    private List<String> getTextLines(String str) {
        return Arrays.asList(str.replaceAll(IOUtils.LINE_SEPARATOR_WINDOWS, StringUtils.f1322CR).replaceAll("\n", StringUtils.f1322CR).split(StringUtils.f1322CR));
    }
    ...
    private float getTextLineWidthForGlyphs(String str, Font font, float f, float f2) {
        ...
        for (int i = 0; i < str.length(); i++) {
            FontCharacter fontCharacter = this.composition.getCharacters().get(FontCharacter.hashFor(str.charAt(i), font.getFamily(), font.getStyle()));
            if (fontCharacter != null) {
                f3 = (float) (((double) f3) + (fontCharacter.getWidth() * ((double) f) * ((double) C0749Utils.dpScale()) * ((double) f2)));
            }
        }
        return f3;
    }
    ...
    static /* synthetic */ class C07443 {
        ...
        static {
            ...
            L_0x0028:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.airbnb.lottie.model.layer.TextLayer.C07443.<clinit>():void");
        }
    }

    private void applyJustification(DocumentData.Justification justification, Canvas canvas, float f) {
        int i = C07443.$SwitchMap$com$airbnb$lottie$model$DocumentData$Justification[justification.ordinal()];
        if (i == 2) {
    ...
    private void drawCharacterAsGlyph(FontCharacter fontCharacter, Matrix matrix2, float f, DocumentData documentData, Canvas canvas) {
        ...
        for (int i = 0; i < contentsForCharacter2.size(); i++) {
            ...
            this.matrix.preTranslate(0.0f, (-documentData.baselineShift) * C0749Utils.dpScale());
