...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f44pm = new PathMeasure();
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        C0692L.beginSection("StrokeContent#draw");
        if (C0749Utils.hasZeroScaleAxis(matrix)) {
            C0692L.endSection("StrokeContent#draw");
            return;
        }
        ...
        this.paint.setStrokeWidth(((FloatKeyframeAnimation) this.widthAnimation).getFloatValue() * C0749Utils.getScale(matrix));
        ...
        for (int i2 = 0; i2 < this.pathGroups.size(); i2++) {
            ...
            if (pathGroup.trimPath != null) {
                ...
                C0692L.beginSection("StrokeContent#buildPath");
                this.path.reset();
                for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
                    this.path.addPath(((PathContent) pathGroup.paths.get(size)).getPath(), matrix);
                }
                C0692L.endSection("StrokeContent#buildPath");
                C0692L.beginSection("StrokeContent#drawPath");
                canvas.drawPath(this.path, this.paint);
                C0692L.endSection("StrokeContent#drawPath");
            }
        }
        C0692L.endSection("StrokeContent#draw");
    }
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        C0692L.beginSection("StrokeContent#applyTrimPath");
        if (pathGroup.trimPath == null) {
            C0692L.endSection("StrokeContent#applyTrimPath");
            return;
        }
        this.path.reset();
        for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
        ...
        this.f44pm.setPath(this.path, false);
        float length = this.f44pm.getLength();
        while (this.f44pm.nextContour()) {
            length += this.f44pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f44pm.setPath(this.trimPathPath, false);
            float length2 = this.f44pm.getLength();
            float f2 = 1.0f;
            if (floatValue3 > length) {
                float f3 = floatValue3 - length;
                if (f3 < f + length2 && f < f3) {
                    C0749Utils.applyTrimPathIfNeeded(this.trimPathPath, floatValue2 > length ? (floatValue2 - length) / length2 : 0.0f, Math.min(f3 / length2, 1.0f), 0.0f);
            ...
            if (f4 >= floatValue2 && f <= floatValue3) {
                if (f4 > floatValue3 || floatValue2 >= f) {
                    ...
                    C0749Utils.applyTrimPathIfNeeded(this.trimPathPath, f5, f2, 0.0f);
                    canvas.drawPath(this.trimPathPath, this.paint);
                } else {
                    canvas.drawPath(this.trimPathPath, this.paint);
                }
    ...
    public void getBounds(RectF rectF, Matrix matrix, boolean z) {
        C0692L.beginSection("StrokeContent#getBounds");
        this.path.reset();
        ...
        C0692L.endSection("StrokeContent#getBounds");
    }

    private void applyDashPatternIfNeeded(Matrix matrix) {
        C0692L.beginSection("StrokeContent#applyDashPattern");
        if (this.dashPatternAnimations.isEmpty()) {
            C0692L.endSection("StrokeContent#applyDashPattern");
            return;
        }
        float scale = C0749Utils.getScale(matrix);
